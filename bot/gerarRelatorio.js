...
function gerarRelatorio(resultados) {
    const pasta = path.join(__dirname, '..', 'relatorio');
    if (!fs.existsSync(pasta)) fs.mkdirSync(pasta);

    const dataHoje = new Date().toLocaleDateString('pt-BR').replace(/\//g, '-');
    const horaAgora = new Date().toLocaleTimeString('pt-BR').replace(/:/g, '-');
    const arquivoPDF = path.join(pasta, `relatorio_${dataHoje}_${horaAgora}.pdf`);
    ...
    blocos.forEach((b, i) => {
        ...
        doc.fillColor('white').fontSize(22).font('Helvetica-Bold').text(b.valor, x, yColunas + 8, { width: 118, align: 'center' });
        doc.fontSize(10).font('Helvetica').text(b.label, x, yColunas + 36, { width: 118, align: 'center' });
    });
    ...
    resultados.forEach((r, idx) => {
        ...
        const corStatus = r.status === 'ENVIADO' ? '#4CAF50' : r.status === 'SEM_NUMERO' ? '#FF9800' : '#F44336';
        ...
        doc.fillColor(corStatus).fontSize(10).font('Helvetica-Bold')
            .text(`Status: ${r.status}`, 300, doc.y - 12);

        if (r.erro) {
            doc.fillColor('#F44336').fontSize(9).font('Helvetica')
        ...
        if (r.print && fs.existsSync(r.print)) {
            try {
                ...
                doc.fillColor('#999').fontSize(9).text('[Não foi possível carregar o print]', 48, doc.y);
            }
        } else {
